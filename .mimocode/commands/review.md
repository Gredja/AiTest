---
description: Full project review against all rules
argument-hint: (no arguments)
---

# Full Project Review

Review the entire Gredja project against all rules and patterns. This is a comprehensive audit — all .cs files, all rules, all config.

## Step 1: Collect project files

Run in parallel:
- List all `.cs` files: `Get-ChildItem -Recurse -Include "*.cs" | Where-Object { $_.FullName -notmatch "\\obj\\" }`
- Read `Rules/*.md` — all rule files
- Read `Core/Config/FakeStoreEndpoints.cs` and `Core/Config/JsonPlaceholderEndpoints.cs`

## Step 2: Spawn subagent

This is the heaviest review operation — full project scan. Delegate to subagent.
