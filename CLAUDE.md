...
### Workflow
Все изменения: план → одобрение → отчёт. После коммита — review `Rules/`. После структурных изменений — обновить документацию. После изменения Rules/ — обновить CLAUDE.md. Выполненные планы (`.mimocode/plans/`) — удалять сразу после реализации.
