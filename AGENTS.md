...
- `/commit` — коммит с safety checks + Allure + push
- `/pr` — создание PR с safety checks
- `/review` — review незакомиченных файлов
...
- `Rules/models.md` — model building rules (Model/Request, properties, naming, attributes)
...
- Attributes: над свойством на отдельной строке, не inline
...
### Workflow
- **Skill-first rule (CRITICAL):** Когда скилл существует — ВСЕГДА загружать и следовать ему (`/commit`, `/test`, `/review-pr`). НИКОГДА не использовать инлайн-инструкции из системного промпта — файл скилла является источником истины.
